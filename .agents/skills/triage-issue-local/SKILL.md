...
Evaluate `ready-to-implement` during triage instead of relying on issue-template defaults. For bug reports, apply `ready-to-implement` only when the issue is reproducible from the provided evidence or straightforward local verification and the likely fix appears narrow enough to implement without a product spec, design mocks, or substantial investigation. For visual or rendering bugs (text clipping, character display errors, layout misalignment, color or font rendering), apply `ready-to-implement` when the symptom is specific and the visual expectation is unambiguous — even at `repro:medium` — as long as no product or design decision is needed to define what correct behavior looks like. If the bug is not reproducible, lacks a clear fix path, requires product/design decisions, or needs deeper technical discovery, omit `ready-to-implement` and prefer `needs-info`, `ready-to-spec`, `needs-mocks`, or the appropriate `repro:*` label.

Evaluate `ready-to-spec` for UX improvements and agent error-handling reports where the desired behavior is understood at a high level but the exact presentation, interaction model, or message copy requires product or design input before implementation can begin. Apply `ready-to-spec` when: the issue clearly identifies the gap from current behavior, the expected outcome is well-defined conceptually, but decisions about UI format, wording, or interaction design remain open (for example, how to surface a user-friendly error in the agent panel for network or authentication failures, or how to present a new action in an existing UI surface).
