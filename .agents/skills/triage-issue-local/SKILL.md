...
For enhancement requests, also apply `ready-to-implement` when the request is well-scoped with clear UX intent, has a narrow implementation surface (for example, adding a built-in theme, a new setting, an `accessibility` affordance, or a small extension to an existing feature), and does not require new product direction or design mocks. Concrete user-facing additions like a built-in high-contrast color theme are typical examples: the desired behavior is unambiguous, the change is bounded, and a maintainer can begin implementation without a product spec. Do not apply `ready-to-implement` to enhancement requests that need substantial product scoping, new design mocks, or cross-surface architectural decisions — prefer `ready-to-spec` or `needs-mocks` in those cases. The `duplicate` label and `ready-to-implement` are not mutually exclusive: a duplicate enhancement can still be marked ready-to-implement so maintainers can pick it up against the canonical issue.
