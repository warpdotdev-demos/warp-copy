...
Evaluate `ready-to-implement` during triage instead of relying on issue-template defaults. For bug reports, apply `ready-to-implement` only when the issue is reproducible from the provided evidence or straightforward local verification and the likely fix appears narrow enough to implement without a product spec, design mocks, or substantial investigation. Localized visual or rendering bugs whose symptom is clearly described in the report — for example, characters clipping at a panel edge, a single layout offset, an off-by-one cursor position, or a stuck focus ring — are typical `ready-to-implement` candidates even when reproducibility is `repro:medium`, because the fix surface is well-scoped and does not require a spec. If the bug is not reproducible, lacks a clear fix path, requires product/design decisions, or needs deeper technical discovery, omit `ready-to-implement` and prefer `needs-info`, `ready-to-spec`, `needs-mocks`, or the appropriate `repro:*` label.

Evaluate `ready-to-spec` during triage as well. Apply `ready-to-spec` when the desired behavior or UX is clearly defined in the report (so there is no ambiguity about *what* should change), but the change still requires a short product or technical spec before implementation can start. Typical examples: user-facing error-message UX (e.g. replacing a raw HTTP/HTML error with a friendly message), new fallback or retry behavior, copy that needs product review, or a new user-visible flow whose surface and edge cases must be enumerated. Prefer `ready-to-spec` over `ready-to-implement` when the report describes the problem and the desired outcome cleanly, but a developer could not pick the issue up and ship a fix without first writing down the intended behavior. Prefer `needs-mocks` instead when the gap is primarily visual design rather than behavior, and `needs-info` when the desired behavior itself is still ambiguous.
