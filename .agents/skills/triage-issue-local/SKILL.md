...
Evaluate `ready-to-implement` and `ready-to-spec` during triage instead of relying on issue-template defaults. Apply at most one of the two per issue, prefer the most specific signal, and otherwise omit both.

- Apply `ready-to-implement` when the issue (bug **or** enhancement) describes the desired behavior concretely enough that a developer could begin work without a separate product spec or design mocks. For bug reports, this means the report is reproducible from the provided evidence or straightforward local verification and the likely fix appears narrow. For enhancement requests, this means the desired UX, scope, and success criteria are explicit in the report (for example, a well-defined visual fix, a clearly described setting, or a self-contained behavior change such as adding a built-in theme or fixing a clipping bug).
- Apply `ready-to-spec` when the issue describes a real problem or user need with clearly stated product expectations, but a small spec or design artifact is still needed before implementation can start. Typical signals: the report names the product-level outcome (for example, a friendly error string, a new surface, or a workflow change) and the design requirements are well-defined, but the exact copy, UX flow, error taxonomy, or component contract still needs to be written down.
- If the bug is not reproducible, lacks a clear fix path, requires product/design decisions that are not yet expressed in the report, or needs deeper technical discovery, omit both labels and prefer `needs-info`, `needs-mocks`, or the appropriate `repro:*` label.
- Do not withhold `ready-to-implement` solely because the issue is also labeled `duplicate` or `enhancement`; evaluate readiness on the merits of the report.
